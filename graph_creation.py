...
def create_plot(title, results, location):
    ...
    # all equality
    plt.plot(x, results[25:30], '-o', color='blue', label="Total Average Equality")
    total = Line2D([0], [0], color='blue', linestyle="-", label="Total Average Equality")

    plt.legend(handles=[total, condition, use_accuracy, overall, statistical, treatment], loc=location, prop={'size': 8})
...
def make_all_unfairness_plots():
    for key, value in DATA_DICT.items():
        ...
        create_plot("Decision Tree - " + key + " - Unfairness", means[60:90], 'upper right')
...
def make_all_accuracy_plots():
    for key, value in DATA_DICT.items():
        ...
        create_plot("Decision Tree - " + key + " - Accuracy", means[60:90], 'lower left')


def calc_mean(results, col):
    means = []
    # hard coded for current number of experiments
    # 100 iterations per unfairness weight + method and 9000 total columns per dataset
    # change if needed
